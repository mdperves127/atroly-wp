...
<!-- Footer Start Here -->
<footer class="footer-area bg--base-four overflow--hidden footer-four" style="background: url(<?php echo esc_url($footer_combine_bg); ?>);">
...
<?php if ( is_active_sidebar('footer-3-1') OR is_active_sidebar('footer-3-2') OR is_active_sidebar('footer-3-3') OR is_active_sidebar('footer-3-4') OR is_active_sidebar('footer-top-3-1') OR is_active_sidebar('footer-top-3-2') OR is_active_sidebar('footer-top-3-3') ): ?>
    <div class="footer-top">
        <div class="container">
            <?php if ( is_active_sidebar('footer-top-3-1') OR is_active_sidebar('footer-top-3-2') OR is_active_sidebar('footer-top-3-3') ): ?>
            <div class="footer-logo--wrap">
                <div class="row justify-content-between gy-4">
                    ...
                    <?php
                            ...
                            dynamic_sidebar( 'footer-top-3-1' );
                            print '</div>';
                            }
                            if ( isset( $footer_top_class[2] ) ) {
                            print '<div class="col-lg-6 wow animate__animated animate__fadeInUp">';
                            dynamic_sidebar( 'footer-top-3-2' );
                            print '</div>';
                            }

                            if ( isset( $footer_top_class[3] ) ) {
                            print '<div class="col-lg-6 wow animate__animated animate__fadeInUp">';
                            dynamic_sidebar( 'footer-top-3-3' );
                            print '</div>';
                            }
                            } else {
                                for ( $num = 1; $num <= $footer_top_columns; $num++ ) {
                                    if ( !is_active_sidebar( 'footer-top-3-' . $num ) ) {
                                        continue;
                                    }
                                    print '<div class="' . esc_attr( $footer_class[$num] ) . '">';
                                    dynamic_sidebar( 'footer-top-3-' . $num );
                                    print '</div>';
                                }
                            }
                        ?>

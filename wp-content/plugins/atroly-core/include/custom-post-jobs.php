...
class AtrolyJobsPost 
{
	...
	public function register_custom_post_type() {
		$technix_jobs_slug = get_theme_mod( 'technix_jobs_slug', __( 'jobs', 'atrolycore' ) );
		$labels = array(
			'name'                  => esc_html_x( 'Jobs', 'Post Type General Name', 'atrolycore' ),
			'singular_name'         => esc_html_x( 'job', 'Post Type Singular Name', 'atrolycore' ),
			'menu_name'             => esc_html__( 'Jobs', 'atrolycore' ),
			'name_admin_bar'        => esc_html__( 'Jobs', 'atrolycore' ),
			...
			'add_new_item'          => esc_html__( 'Add New job', 'atrolycore' ),
		...
		$args   = array(
			'label'                 => esc_html__( 'job', 'atrolycore' ),
			'labels'                => $labels,
	...
	public function create_cat() {
		$labels = array(
			'name'                       => esc_html_x( 'Job Categories', 'Taxonomy General Name', 'atrolycore' ),
			'singular_name'              => esc_html_x( 'Job Categories', 'Taxonomy Singular Name', 'atrolycore' ),
			'menu_name'                  => esc_html__( 'Job Categories', 'atrolycore' ),
			'all_items'                  => esc_html__( 'All job Category', 'atrolycore' ),
			'parent_item'                => esc_html__( 'Parent Item', 'atrolycore' ),
			'parent_item_colon'          => esc_html__( 'Parent Item:', 'atrolycore' ),
			'new_item_name'              => esc_html__( 'New job Category Name', 'atrolycore' ),
			'add_new_item'               => esc_html__( 'Add New job Category', 'atrolycore' ),
			'edit_item'                  => esc_html__( 'Edit job Category', 'atrolycore' ),
			'update_item'                => esc_html__( 'Update job Category', 'atrolycore' ),
			'view_item'                  => esc_html__( 'View job Category', 'atrolycore' ),
			...
			'popular_items'              => esc_html__( 'Popular job Category', 'atrolycore' ),
			'search_items'               => esc_html__( 'Search job Category', 'atrolycore' ),
			'not_found'                  => esc_html__( 'Not Found', 'atrolycore' ),
			'no_terms'                   => esc_html__( 'No job Category', 'atrolycore' ),
			'items_list'                 => esc_html__( 'job Category list', 'atrolycore' ),
			'items_list_navigation'      => esc_html__( 'job Category list navigation', 'atrolycore' ),
		);
...
function custom_jobs_func( $post ) {
	...
	<div class="job-additional-info">
		<div class="job-type">
			<h4>Job Type</h4>
			<input class="widefat" type="text" name="atroly-job-type" placeholder="add job type" value="<?php if(isset($meta_id['atroly-job-type'])) echo $meta_id['atroly-job-type'][0]; ?>" >
		</div>
		<div class="job-location">
			<h4>Job Location</h4>
			<input class="widefat" type="text" name="atroly-job-location" placeholder="add job location" value="<?php if(isset($meta_id['atroly-job-location'])) echo $meta_id['atroly-job-location'][0]; ?>" >
		</div>
	</div>
	<?php
}
...
add_action('save_post', function($post_id){
	if(isset($_POST['atroly-job-type'])){
		update_post_meta($post_id, 'atroly-job-type', $_POST['atroly-job-type']);
	};
	if(isset($_POST['atroly-job-location'])){
		update_post_meta($post_id, 'atroly-job-location', $_POST['atroly-job-location']);
	};
});

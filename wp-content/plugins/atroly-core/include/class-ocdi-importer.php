...
function atroly_ocdi_import_files() {
    ...
    return array(
      array(
        ...
        'preview_url'                => 'https://wp.storebuild.shop/atroly/',
      ),
      array(
        ...
        'preview_url'                => 'https://wp.storebuild.shop/atroly/home-02/',
      ),
      array(
        ...
        'preview_url'                => 'https://wp.storebuild.shop/atroly/home-03/',
      ),
    );
}
add_filter( 'ocdi/import_files', 'atroly_ocdi_import_files' );


function atroly_ocdi_page($atroly_page_name = 'Home'){
    $posts = get_posts(
        array(
            'post_type'              => 'page',
            'title'                  => $atroly_page_name,
            'post_status'            => 'all',
            'posts_per_page'         => 1,
            'no_found_rows'          => true,
...
// after demo imports
function atroly_ocdi_after_import_setup( $demo ) {
    $front_page_id = "";
    $blog_page_id = "";
    if( "Home 1" == $demo['import_file_name'] ){
        // Assign front page and posts page (blog page).
        $front_page_id = atroly_ocdi_page( 'Home' );
        $blog_page_id  = atroly_ocdi_page( 'Blog' );
    }else if( "Home 2" == $demo['import_file_name'] ){
        // Assign front page and posts page (blog page).
        $front_page_id = atroly_ocdi_page( 'Home 02' );
        $blog_page_id  = atroly_ocdi_page( 'Blog' );
    }
    else if( "Home 3" == $demo['import_file_name'] ){
        // Assign front page and posts page (blog page).
        $front_page_id = atroly_ocdi_page( 'Home 03' );
        $blog_page_id  = atroly_ocdi_page( 'Blog' );
    }
...
add_action( 'ocdi/after_import', 'atroly_ocdi_after_import_setup' );



function atroly_ocdi_plugin_page_setup( $default_settings ) {
...
add_filter( 'ocdi/plugin_page_setup', 'atroly_ocdi_plugin_page_setup' );
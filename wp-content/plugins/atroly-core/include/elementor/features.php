 ...
 * @since 1.0.0
 */
class ATROLY_Features extends Widget_Base {
	...
	protected function register_controls_section() {
        ...
        // title/content
        $this->atroly_section_title_render_controls('section', 'Section Title', 'Sub Title', 'your title here', $default_description = 'Hic nesciunt galisum aut dolorem aperiam eum soluta quod ea cupiditate.' );
        ...
        $repeater->add_control(
            ...
            [
                ...
                'condition' => [
                    'atroly_features_icon_type' => 'image',
                ]
        ...
        if (atroly_is_elementor_version('<', '2.6.0')) {
            $repeater->add_control(
                ...
                [
                    ...
                    'default' => 'fa fa-star',
                    'condition' => [
                        'atroly_features_icon_type' => 'icon'
                    ]
                ]
            );
        } else {
            $repeater->add_control(
                'atroly_features_selected_icon',
                [
                    'show_label' => false,
        ...
        $repeater->add_control(
            ...
            [
                ...
                'label_block' => true,
                'condition' => [
                    'atroly_features_link_switcher' => 'yes',
                    'repeater_condition' => 'style_2'
                ],
            ]
        );
        $repeater->add_control(
            'atroly_features_link_type',
            [
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => [
                    [
                        'atroly_features_title' => esc_html__('Discover', 'atrolycore'),
                    ],
                    [
                        'atroly_features_title' => esc_html__('Define', 'atrolycore')
                    ],
                    [
                        'atroly_features_title' => esc_html__('Develop', 'atrolycore')
                    ]
                ],
    ...
    protected function style_tab_content(){
        $this->atroly_section_style_controls('features_section', 'Section - Style', '.atroly-el-section'); 
    }
...
<?php else: 
    ...
    $shapeClass = $settings['atroly_about_shape_switch'] ? NULL : 'feature-breadcrumb' ;
    
    $this->add_render_attribute('title_args', 'class', 'atroly-section-title'); 
?>

<section class="atroly-feature-area atroly-el-section atroly-el-section <?php echo esc_attr($shapeClass); ?>">
    <?php if(!empty($atroly_shape_image)) : ?>
    <div class="atroly-feature-shape">
    ...
    <div class="container container-large">
        ...
        <div class="row align-items-center">
            <div class="col-lg-6">
                <div class="atroly-feature-title-wrapper">
                    <?php if ( !empty($settings['atroly_section_sub_title']) ) : ?>
                    <span class="atroly-section-title__pre"><?php echo atroly_kses( $settings['atroly_section_sub_title'] ); ?></span>
                    <?php endif; ?>
                    <?php
            ...
            <div class="col-lg-6">
                ...
                <div class="atroly-feature-wrapper p-relative">
        ...
        <div class="row">
            ...
            <?php foreach ($settings['atroly_features_list'] as $key => $item) :
                // Link
                if ('2' == $item['atroly_features_link_type']) {
                    $link = get_permalink($item['atroly_features_page_link']);
                    $target = '_self';
                    $rel = 'nofollow';
                } else {
                    $link = !empty($item['atroly_features_link']['url']) ? $item['atroly_features_link']['url'] : '';
                    $target = !empty($item['atroly_features_link']['is_external']) ? '_blank' : '';
                    $rel = !empty($item['atroly_features_link']['nofollow']) ? 'nofollow' : '';
                }

                // thumbnail image
                if ( !empty($item['atroly_features_main_image']['url']) ) {
                    $atroly_features_main_image = !empty($item['atroly_features_main_image']['id']) ? wp_get_attachment_image_url( $item['atroly_features_main_image']['id'], 'full' ) : $item['atroly_features_main_image']['url'];
                    $atroly_features_main_image_alt = get_post_meta($item["atroly_features_main_image"]["id"], "_wp_attachment_image_alt", true);
                }
            ...
            <div class="col-xl-<?php echo esc_attr($settings['atroly_col_for_desktop']); ?> col-lg-<?php echo esc_attr($settings['atroly_col_for_laptop']); ?> col-md-<?php echo esc_attr($settings['atroly_col_for_tablet']); ?> col-<?php echo esc_attr($settings['atroly_col_for_mobile']); ?>">
                ...
                <div class="atroly-feature-item-box p-relative wow <?php echo esc_attr($item['atroly_anima_type']); ?>" data-wow-duration="<?php echo esc_attr($item['atroly_anima_dura']); ?>" data-wow-delay="<?php echo esc_attr($item['atroly_anima_delay']); ?>">
                <?php else : ?>
                <div class="atroly-feature-item-box p-relative">
                <?php endif; ?>
                    <div class="atroly-feature-item p-relative mb-30">
                        <div class="atroly-feature-item-shape">
                            <img src="<?php echo get_template_directory_uri(); ?>/assets/img/feature/shape-2.png" alt="">
                        </div>
                        <div class="atroly-feature-item-wrapper">
                            <div class="atroly-feature-item-thumb">

                                <?php if($item['atroly_features_icon_type'] == 'icon') : ?>
                                <?php if (!empty($item['atroly_features_icon']) || !empty($item['atroly_features_selected_icon']['value'])) : ?>
                                <div class="shape">
                                    <?php atroly_render_icon($item, 'atroly_features_icon', 'atroly_features_selected_icon'); ?>
                                </div>
                                <?php endif; ?>
                                <?php elseif( $item['atroly_features_icon_type'] == 'image' ) : ?>
                                <?php if (!empty($item['atroly_features_image']['url'])): ?>
                                <div class="shape">
                                    <img src="<?php echo $item['atroly_features_image']['url']; ?>"
                                        alt="<?php echo get_post_meta(attachment_url_to_postid($item['atroly_features_image']['url']), '_wp_attachment_image_alt', true); ?>">
                                </div>
                                <?php endif; ?>
                                <?php else : ?>
                                <?php if (!empty($item['atroly_features_icon_svg'])): ?>
                                <div class="shape">
                                    <?php echo $item['atroly_features_icon_svg']; ?>
                                </div>
                                <?php endif; ?>
                                <?php endif; ?>

                                <?php if(!empty($atroly_features_main_image)) : ?>
                                    <img class="thumb" src="<?php echo esc_url($atroly_features_main_image); ?>" alt="<?php echo esc_attr($atroly_features_main_image_alt); ?>">
                                <?php endif; ?>
                            </div>
                            <div class="atroly-feature-item-content">

                                <?php if (!empty($item['atroly_features_title' ])): ?>
                                <h3 class="feature-title">
                                    <?php if (!empty($link)) : ?>
                                    <a href="<?php echo esc_url($link); ?>"><?php echo atroly_kses($item['atroly_features_title' ]); ?></a>
                                    <?php else : ?>
                                    <?php echo atroly_kses($item['atroly_features_title' ]); ?>
                                    <?php endif; ?>
                                <span><svg width="130" height="8" viewBox="0 0 130 8" fill="none" xmlns="htatroly://www.w3.org/2000/svg">
                                ...
                                <?php endif; ?>
                                <?php if (!empty($item['atroly_features_description' ])): ?>
                                <p><?php echo atroly_kses($item['atroly_features_description']); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    <?php if(!empty($link)) : ?>
                    <div class="atroly-feature-item-btn">
...
$widgets_manager->register( new ATROLY_Features() );
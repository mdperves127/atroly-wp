!function($, $ele, $i) {
    window.liteTranslated = function (e, t) {
        return elementorCommon.translate(e, null, t, atrolycoreEditor.i18n)
    };
    ...
    $app.Views.EmptyTemplateCollection = Marionette.ItemView.extend({
        ...
        template: "#atrolycore-liteTemplateLibrary_empty",
        ui: {
    ...
    $app.Views.Loading = Marionette.ItemView.extend({
        template: "#atrolycore-liteTemplateLibrary_loading",
        id: "liteTemplateLibrary_loading"
    }), 
    $app.Views.Logo = Marionette.ItemView.extend({
        template: "#atrolycore-harry-template-library-header-logo",
    ...
    $app.Views.BackButton = Marionette.ItemView.extend({
        template: "#atrolycore-liteTemplateLibrary_header-back",
    ...
    $app.Views.Menu = Marionette.ItemView.extend({
        template: "#atrolycore-liteTemplateLibrary_header-menu",
    ...
    $app.Views.ResponsiveMenu = Marionette.ItemView.extend({
        template: "#atrolycore-liteTemplateLibrary_header-menu-responsive",
    ...
    $app.Views.Actions = Marionette.ItemView.extend({
        template: "#atrolycore-liteTemplateLibrary_header-actions",
    ...
    $app.Views.InsertWrapper = Marionette.ItemView.extend({
        template: "#atrolycore-liteTemplateLibrary_header-insert",
    ...
    $app.Views.Preview = Marionette.ItemView.extend({
        template: "#atrolycore-liteTemplateLibrary_preview",
    ...
    $app.Views.TemplateCollection = Marionette.CompositeView.extend({
        template: "#atrolycore-liteTemplateLibrary_templates",
    ...
    $app.Views.Template = Marionette.ItemView.extend({
        template: "#atrolycore-liteTemplateLibrary_template",
    ...
    $app.Modal = elementorModules.common.views.modal.Layout.extend({
        ...
        getTemplateActionButton: function(e) {
            var t = e.isPro && !atrolycoreEditor.hasPro ? "pro-button" : "insert-button";
            return viewId = "#atrolycore-liteTemplateLibrary_" + t, template = Marionette.TemplateCache.get(viewId), Marionette.Renderer.render(template)
        },
        showLogo: function(e) {
    ...
    $app.Manager = function() {
        ...
        function addButton() {
            ...
            }), $sl.find(".atroly_templates_add_button").length || $sl.find(DROIT_FIND_SELECTOR).before($atrolycorePpenLibraryButton)
        }

        function initAddButton($el) {
            var $t = $el.find(DROIT_FIND_SELECTOR);
            $t.length && !$el.find(".atroly_templates_add_button").length && $t.before($atrolycorePpenLibraryButton), $el.on("click.onAddElement", ".elementor-editor-section-settings .elementor-editor-element-add", addButton)
        }

        function onChanges(t, i) {
            i.addClass("elementor-active").siblings().removeClass("elementor-active");
            var a = $atrolyResponsiveMap[t] || $atrolyResponsiveMap.desktop;
        ...
        function onEditorLoaded() {
            var $el = window.elementor.$previewContents,
                ...
                $el.on("click.onAddTemplateButton", ".atroly_templates_add_button", $this.showModal.bind($this)), this.channels.tabs.on("change:device", onChanges)
        }
        ...
        $atrolycorePpenLibraryButton = '<div class="elementor-add-section-area-button atroly_templates_add_button"> <img src="'+atrolycoreEditor.templateLogo+'" /> </div>', 
        $atrolyResponsiveMap = {
            desktop: "100%",
            tab: "768px",
            mobile: "360px"
        ...
        this.init = function() {
            $this.setFilter("type", atrolycoreEditor.default_tab, !0), $ele.on("preview:loaded", onEditorLoaded.bind(this))
        }, 
        this.getTabs = function() {
            var e = this.getFilter("type");
            return tabs = JSON.parse(atrolycoreEditor.tab_style)
        ...
        this.requestLibraryData = function(e) {
            ...
            e.forceSync && (t.data.sync = !0), elementorCommon.ajax.addRequest("get_atrolycore_library_data", t)
        }, 
        this.requestTemplateData = function(e, t) {
            ...
            t && jQuery.extend(!0, i, t), elementorCommon.ajax.addRequest("get_atrolycore_template_data", i)
        }, 
...
}(jQuery, window.elementor, window.atrolycore || {});

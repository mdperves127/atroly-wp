...
use ATROLY_ELEMENTOR\Templates\ATROLY_Api as TemplatesATROLY_Api;
...
class ATROLY_Api extends Source_Base {

	public function get_id() {
		return 'atroly-library';
	}

	public function get_title() {
		return __( 'ATROLY Addons Library', 'atrolycore' );
	}
	...
	private static function request_library_data( $force_update = false ) {
		$response	=	wp_remote_get(ATROLY_API_URL .'data.json',['timeout'=>60]);
	...
	public static function request_template_data( $template_id ) {
		...
		$template_url = sprintf(ATROLY_API_URL . "templates/%s.json", $template_id);

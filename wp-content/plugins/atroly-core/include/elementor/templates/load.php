...
use \ATROLY_ELEMENTOR\Templates\ATROLY_Api;
use \ATROLY_ELEMENTOR\Templates\ATROLY_Templates as ATROLY_init;
...
class ATROLY_Load{
    ...
    public function load_template_views() {
        include ATROLY_init::dir() . 'templates.php';
    }
   
    public static function get_library() {
        if ( is_null( self::$library_data ) ) {
            self::$library_data = new ATROLY_Api();
        }
        return self::$library_data;
    }
 
    public function register_ajax_actions( Ajax $ajax ) {
        $ajax->register_ajax_action( 'get_atrolycore_library_data', function( $data ) {
